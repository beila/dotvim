...
let &makeprg = 'vs12build c:\igbuild\project\Ignition.sln RelWithDebInfo /build'
map <F11> :wa<Enter>:make<Enter>:TagbarClose<Enter>:copen<Enter><Enter>
map <S-F11> :silent !orbis-run /workingDirectory:c:\igbuild\install /spawn /noprogress /elf c:\igbuild\install\bin\ignition_monolith.elf --plugin=com.amazon.ignition.app.stark --enable-renderer-shortcuts --loglevel=info --js-args="--fake-player --skip-connectivity-check"<Enter><Enter>
